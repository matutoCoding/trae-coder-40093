...
import { useCallRecordStore } from './callRecordStore';
import { usePharmacistTaskStore } from './pharmacistTaskStore';
...
function computeStats(): DashboardStats {
  const callTasks = useCallTaskStore.getState().callTasks;
  const callRecords = useCallRecordStore.getState().callRecords;
  const pharmacistTasks = usePharmacistTaskStore.getState().pharmacistTasks;
...
export const useDashboardStore = create<DashboardState>((set) => ({
  stats: computeStats(),
  refreshStats: () => set({ stats: computeStats() }),
}));
